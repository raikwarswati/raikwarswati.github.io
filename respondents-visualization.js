...
let config = {
  ...
  // imagePath: 'assets/data/images/images.png',
  imagePath: ''
};

let respondentsData = null;

// Helper to normalize keys (moved to top-level scope)
...
// Track the current order of respondent indices for animation
...
async function loadRespondentsData() {
  try {
    ...
    const response = await fetch('respondents_data_all.json');
    ...
    config.colors = {
      generations: generateCategoryColors(
        Q3_GENERATION_ORDER,
    ...
    // Normalize the keys for generations color mapping
    const normalizedGenColors = {};
    Object.entries(config.colors.generations).forEach(([k, v]) => {
      normalizedGenColors[normalize(k)] = v;
    });
    config.colors.generations = normalizedGenColors;
    
    console.log('Config generated:', config);

    try {
      // Load the tile image using the path from config
      if (config.imagePath) {
        console.log('Loading tile image from:', config.imagePath);
        await loadTileImage(config.imagePath);
        console.log('Tile image loaded successfully');
      } else {
        console.warn('No image path specified in config');
      }
    } catch (imageError) {
      console.error('Failed to load tile image:', imageError);
      console.warn('Falling back to colored rectangles');
    }
    
    // Initialize visualization after loading both data and image
...
function initRespondentsVisualization() {
  ...
  controls.innerHTML = `
    ...
    <button class="control-btn" data-action="sort" data-sort="generations">Sort by Generation</button>
  ...
  // Initialize currentOrder to the original order
  currentOrder = Object.keys(respondentsData).map(Number);
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    if (index >= 0 && index < config.totalRespondents) {
      const respondentIdx = currentOrder ? currentOrder[index] : index;
      const respondent = respondentsData[respondentIdx];
      if (respondent) {
  ...
  canvas.addEventListener('click', (e) => {
    ...
    if (index >= 0 && index < config.totalRespondents) {
      const respondentIdx = currentOrder ? currentOrder[index] : index;
      displayRespondentData(respondentIdx);
      modal.style.display = 'flex';
      // Store current index
      modal.dataset.currentIndex = respondentIdx;
    }
  });
...
// Load and cache image for tiles
function loadTileImage(imagePath) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
      config.tileImage = img;
...
function drawTiles(ctx) {
  ...
  const { tileSize, padding, totalRespondents, tileImage } = config;
  ...
  // Use currentOrder to draw tiles in their current order
  for (let i = 0; i < totalRespondents; i++) {
    const respondentIndex = currentOrder ? currentOrder[i] : i;
...
function groupTiles(ctx, category) {
  ...
  const { tileSize, padding, totalRespondents, colors, tileImage } = config;
  const canvas = ctx.canvas;
  const cols = Math.floor(canvas.width / (tileSize + padding));
  const rows = Math.ceil(totalRespondents / cols);
  ...
  Object.entries(respondentsData).forEach(([index, data], i) => {
    ...
    if (tileImage) {
      ...
      // Apply color tint based on category
      ctx.save();
      ...
      switch(category) {
        case 'generations': {
          const genKey = normalize(data.Q3_generation);
      ...
      // Fallback to original colored rectangle behavior
      let color = config.theme.accent;
      switch(category) {
        case 'generations': {
          const genKey = normalize(data.Q3_generation);
  ...
  legend.innerHTML = order
    .map(group => `
      <div class="legend-item">
        <span class="color-box" style="background-color: ${colors[normalize(group)]}"></span>
...
// Function to sort and animate tiles
function sortTiles(ctx, category) {
  console.log('Sorting tiles by:', category);
  const { tileSize, padding, totalRespondents, colors, tileImage } = config;
  const canvas = ctx.canvas;
  const cols = Math.floor(canvas.width / (tileSize + padding));

  // 1. Create a sorted order of indices based on generation, using normalization
  const normalizedOrder = Q3_GENERATION_ORDER.map(normalize);
  const sortedOrder = currentOrder.slice().sort((aIdx, bIdx) => {
    const a = respondentsData[aIdx];
    const b = respondentsData[bIdx];
    const aNorm = normalize(a.Q3_generation);
    const bNorm = normalize(b.Q3_generation);
    return normalizedOrder.indexOf(aNorm) - normalizedOrder.indexOf(bNorm);
  });

  // 2. Animate each tile from its current position to its new position
  ...
  function animate(currentTime) {
    ...
    for (let i = 0; i < totalRespondents; i++) {
      const respondentIdx = currentOrder[i];
      const targetIdx = sortedOrder.indexOf(respondentIdx);

      // Current grid position
      const currentRow = Math.floor(i / cols);
      const currentCol = i % cols;
      const startX = currentCol * (tileSize + padding);
      const startY = currentRow * (tileSize + padding);

      // Target grid position
      const targetRow = Math.floor(targetIdx / cols);
      const targetCol = targetIdx % cols;
      const endX = targetCol * (tileSize + padding);
      const endY = targetRow * (tileSize + padding);
      ...
      if (tileImage) {
        ...
        // Apply generation color tint
        ctx.save();
        ctx.globalCompositeOperation = 'multiply';
        const respondent = respondentsData[respondentIdx];
        const genKey = normalize(respondent.Q3_generation);
        const color = colors.generations[genKey] || config.theme.accent;
        ctx.fillStyle = color;
        ctx.fillRect(x, y, tileSize, tileSize);
        ctx.restore();
      } else {
    ...
    if (progress < 1) {
      ...
      // 3. After animation, update currentOrder to the new sorted order
      currentOrder = sortedOrder;
      updateLegend('generations');
    }
  }
  requestAnimationFrame(animate);
}
